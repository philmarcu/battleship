...
RSpec.describe Board do
  ...
  it 'will only allow consecutive coords, not diagnol' do
    expect(board.valid_placement?(cruiser, ["B1", "C1", "D1"])).to eq(false) #fail diagnol test for cruiser
    ...
    expect(board.valid_placement?(submarine, ["C2", "D3"])).to eq(false)  # fail diagnol for sub
